...
public class reportesVenta extends JFrame implements ActionListener {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					reportesVenta frame = new reportesVenta();
	...
	public reportesVenta() {
		...
		btnOpen.setIcon(new ImageIcon(reportesVenta.class.getResource("/IMG/open.png")));
		...
		btnBuscar.setIcon(new ImageIcon(reportesVenta.class.getResource("/IMG/bigSearch.png")));
	...
	private ArrayList<Object> cargarColumnas() {
		ArrayList<Object> lista = gBoleta.leerColumnas("venta");
		return lista;
	}

	private ArrayList<Boleta> cargarBoletas() {
		ArrayList<Boleta> lista = gBoleta.leerBoleta("venta");
		return lista;
	}
	...
	public void construirTablaPorFecha(String fecha) {
		listaBoleta = gBoleta.buscarBoleta(fecha, "venta");
		listaTitulos = cargarColumnas();
	...
	protected void do_btnOpen_actionPerformed(ActionEvent e) {
		...
		if (rowNum != -1) {
			...
			File carpeta = new File("Reportes/ReporteVenta");
			String[] filesList = carpeta.list();
			for (String fList : filesList) {
				if (fList.contains(codeFinal)) {
					try {
						condicion = false;
						Desktop.getDesktop().open(new File("Reportes/ReporteVenta/" + fList));
			...
			if(condicion) {
				...
				if(result == 0) {
					...
					mR.setMessaje("Reporte Virtual",  
							...
							+ "Codigo Cliente: "+boleta.getCod_Cli_Prov()+"\n"
