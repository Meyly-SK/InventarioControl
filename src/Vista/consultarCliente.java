...
public class consultarCliente extends JInternalFrame implements ActionListener, MouseListener, FocusListener,
	...
	public consultarCliente(frmPrincipal principal) {
		this();
	...
	public consultarCliente() {
		...
		lblCountEmp = new JLabel("Numero de Clientes Registrados:  ");
		...
		lblNewLabel_13 = new JLabel("FILTRAR CLIENTES");
	...
	private ArrayList<Clientes> cargarClientes(String filtro) {
		ArrayList<Clientes> lista = gCliente.buscarClientes(filtro, numVisibleComponent, listaFiltro);
		return lista;
	}

	private void construirTablaFiltro() {
		listaPersonas = cargarClientes(generarFiltro());
		...
		lblCountEmp.setText("Numero de Clientes Encontrados:  " + tbClientes.getRowCount() + "   ");
	}
	...
	private void construirTabla(String[] titulos, Object[][] data, typeUtility util) {
		...
		tbClientes.getColumnModel().getColumn(util.getIr_a()).setCellRenderer(new editCells("icono"));
		...
		tbClientes.getColumnModel().getColumn(util.getIr_a()).setPreferredWidth(30);
	...
	protected void do_tbClientes_mouseClicked(MouseEvent e) {
		int fila = tbClientes.rowAtPoint(e.getPoint());
		int columna = tbClientes.columnAtPoint(e.getPoint());
		tbClientes.changeSelection(fila, columna, false, false);
		String code = (String) tbClientes.getValueAt(tbClientes.getSelectedRow(), 0);
		Object[] obj = { fila, columna, code };

		if (columna == 11) {
			principal.modi(obj, "regiCliente");
		}
	}
