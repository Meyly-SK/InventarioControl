...
public class generarVenta extends JInternalFrame
	...
	private static JLabel lblCodCliente;
	private static JLabel lblNomCliente;
	private static JLabel lblDniCliente;
	...
	public generarVenta() {
		...
		btnOpenInventory = new JButton("\u2500\u2500 ABRIR INVENTARIO \u2500\u2500");
		...
		btnProceder = new JButton("\u2500\u2500 PROCEDER CON LA VENTA  \u2500\u2500");
		...
		pnCboCategoria = new JPanel();
		pnCboCategoria.setBorder(new TitledBorder(new MatteBorder(1, 1, 1, 1, (Color) new Color(0, 0, 0)), "Categorias",
				TitledBorder.LEFT, TitledBorder.TOP, null, new Color(0, 0, 0)));
		pnCboCategoria.setOpaque(false);
		panel_18.add(pnCboCategoria);
		pnCboCategoria.setLayout(new FlowLayout(FlowLayout.CENTER, 5, 5));
		...
		pnCboCategoria.add(cboCategoria);
		...
		panel_15.add(pnCliente);
		pnCliente.setPreferredSize(new Dimension(280, 207));
		pnCliente.setSize(new Dimension(280, 207));
		...
		panel = new JPanel();
		pnCliente.add(panel, BorderLayout.CENTER);
		...
		btnSelectCliente = new JButton("SELECCIONAR....");
		btnSelectCliente.addActionListener(this);
		btnSelectCliente.addMouseListener(this);
		btnSelectCliente.setBounds(88, 4, 170, 23);
		btnSelectCliente.setFocusable(false);
		btnSelectCliente.setBorder(new MatteBorder(1, 0, 1, 0, (Color) new Color(0, 0, 0)));
		btnSelectCliente.setContentAreaFilled(false);
		panel_2.add(btnSelectCliente);
		btnSelectCliente.setMargin(new Insets(2, 2, 2, 2));
		...
		lblCodCliente.setBounds(88, 4, 145, 22);
		lblCodCliente.setFont(new Font("Tahoma", Font.PLAIN, 13));
		panel_3.add(lblCodCliente);
		...
		lblNewLabel_2.setBounds(7, 8, 46, 14);
		lblNewLabel_2.setFont(new Font("Tahoma", Font.BOLD, 11));
		panel_4.add(lblNewLabel_2);

		lblNomCliente = new JLabel("");
		lblNomCliente.setBounds(88, 4, 145, 22);
		lblNomCliente.setFont(new Font("Tahoma", Font.PLAIN, 11));
		panel_4.add(lblNomCliente);
		...
		lblNewLabel_3.setBounds(7, 8, 20, 14);
		lblNewLabel_3.setFont(new Font("Tahoma", Font.BOLD, 11));
		panel_5.add(lblNewLabel_3);

		lblDniCliente = new JLabel("");
		lblDniCliente.setFont(new Font("Tahoma", Font.PLAIN, 13));
		lblDniCliente.setBounds(88, 4, 145, 22);
		panel_5.add(lblDniCliente);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btnSelectCliente) {
			do_btnSelectCliente_actionPerformed(e);
		}
	...
	private void do_cboCategoria_actionPerformed(ActionEvent e) {
		String categoria = cboCategoria.getSelectedItem() + "";
		if (categoria.equals("SELECCIONAR....")) {
			cboProducto.removeAllItems();
			pnCboProducto.setVisible(false);
		} else {
	...
	public void mouseEntered(MouseEvent e) {
		...
		if (e.getSource() == btnSelectCliente) {
			do_btnSelectCliente_mouseEntered(e);
		}
	...
	public void mouseExited(MouseEvent e) {
		if (e.getSource() == btnSelectCliente) {
			do_btnSelectCliente_mouseExited(e);
		}
	...
	protected void do_btnSelectCliente_mouseEntered(MouseEvent e) {
		if (btnSelectCliente.isEnabled()) {
			btnSelectCliente.setBorder(new MatteBorder(1, 0, 1, 0, (Color.GREEN)));
		}
	}

	private void do_btnSelectCliente_mouseExited(MouseEvent e) {
		if (btnSelectCliente.isEnabled()) {
			btnSelectCliente.setBorder(new MatteBorder(1, 0, 1, 0, (Color) new Color(0, 0, 0)));
		}
	}
	...
	public static void cargarCliente() {
		Clientes cli = frmPrincipal.cliente;
		lblCodCliente.setText(cli.getCodigo());
		lblNomCliente.setText((cli.getNombre() + " " + cli.getApellido()).toUpperCase());
		lblDniCliente.setText(cli.getDni());
	}

	protected void do_btnSelectCliente_actionPerformed(ActionEvent e) {
		seleccionarCliente sC = new seleccionarCliente();
		sC.setVisible(true);
	}

	protected void do_btnProceder_actionPerformed(ActionEvent e) {
		if (!lblCodCliente.getText().trim().equals("")) {
			...
			if (modeling.getRowCount() == 0) {
				...
				if (continuar) {
					...
					if (detList != null && !(detPdf.size() == 0)) {
						...
						bol.setCod_Cli_Prov(frmPrincipal.cliente.getCodigo_()+"");
						...
						try {
							int result = gBoleta.registrarBoleta(bol, detList, "venta");
							if (result != 0) {
								String nombPDF = "Reportes/ReporteVenta/Venta_" + frmPrincipal.trabajador.getCodigo()
										+ "_" + coding + ".pdf";
								...
								JOptionPane.showMessageDialog(null, "Ha ocurrido un error con la venta", "Informacion",
										JOptionPane.WARNING_MESSAGE);
							}
	...
	private ArrayList<String[]> detalleListaProductos() {
		...
		for (int i = 0; i < rowsCount; i++) {
			...
			int resta = Integer.parseInt(tbList.getValueAt(i, 3) + "") - Integer.parseInt(tbList.getValueAt(i, 4) + "");
			if (!(resta < 0)) {
				row[1] = String.valueOf(resta);
				val = true;
				listaP.add(row);
			} else {
				val = false;
	...
	public Object getFieldValue(JRField jrf) throws JRException {
		...
		case "externoDato":
			return value = (frmPrincipal.cliente.getNombre()+" "+frmPrincipal.cliente.getApellido()).toUpperCase();
			
		case "externoNombre":
			return value = "CLIENTE";
			
		case "identidadNum":
			return value = frmPrincipal.cliente.getDni();
			
		case "identidadNombre":
			return value = "DNI";
			
		case "Subtitulo":
			return value = "REPORTE DE VENTA";
