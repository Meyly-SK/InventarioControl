...
public class registrarCliente extends JInternalFrame
	...
	tableModel modelo;
	ArrayList<Clientes> listaPersonas;
	...
	public registrarCliente() {
		setFrameIcon(new ImageIcon(registrarCliente.class.getResource("/IMG/register.png")));
		...
		tbClientes.setModel(new DefaultTableModel(new Object[][] {}, new String[] { "New column" }));
		...
		lblCountCli = new JLabel("Numero de Clientes Registrados:  ");
		panel.add(lblCountCli, BorderLayout.EAST);
		...
		btnNuevo.setIcon(new ImageIcon(registrarCliente.class.getResource("/IMG/new.png")));
		...
		btnEliminar.setIcon(new ImageIcon(registrarCliente.class.getResource("/IMG/delete.png")));
		...
		btnActualizar.setIcon(new ImageIcon(registrarCliente.class.getResource("/IMG/edit.png")));
		...
		btnRefresh.setIcon(new ImageIcon(registrarCliente.class.getResource("/IMG/refresh.png")));
	...
	protected void do_btnRegistrar_actionPerformed(ActionEvent e) {
		ModifyThread mT = new ModifyThread(this, "registroCliente");
		...
		if (registro) {
			...
			btnActualizar.setEnabled(true);
			btnRefresh.setEnabled(true);
			ModifyThread mT2 = new ModifyThread(this, "construirtablaCli");
	...
	private ArrayList<Object> cargarColumnas() {
		ArrayList<Object> lista = gCliente.leerColumnas();
		return lista;
	}

	private ArrayList<Clientes> cargarClientes(int nume) {
		ArrayList<Clientes> lista = gCliente.leerClientes(nume, conditionOrder);
		return lista;
	}
	...
	public void construirTabla(int nume) {
		...
		lblCountCli.setText("Numero de Clientes Registrados:  " + tbClientes.getRowCount());
	}

	public void construirTablaPorCodigo(String code) {
		listaPersonas = gCliente.busquedaPorCodigo(code);
		listaTitulos = cargarColumnas();
	...
	private void construirTabla(String[] titulos, Object[][] data, typeUtility util) {
		...
		tbClientes.getColumnModel().getColumn(util.getEdad()).setCellRenderer(new editCells("numerico"));
		tbClientes.getColumnModel().getColumn(util.getFechaNac()).setCellRenderer(new editCells("numerico"));
		tbClientes.getColumnModel().getColumn(util.getGenero()).setCellRenderer(new editCells("numerico"));
		tbClientes.getColumnModel().getColumn(util.getEstadoCivil()).setCellRenderer(new editCells("texto"));
		tbClientes.getColumnModel().getColumn(util.getCelular()).setCellRenderer(new editCells("numerico"));
		tbClientes.getColumnModel().getColumn(util.getCorreo()).setCellRenderer(new editCells("texto"));
		tbClientes.getColumnModel().getColumn(util.getDireccion()).setCellRenderer(new editCells("texto"));
		...
		tbClientes.getColumnModel().getColumn(util.getEdad()).setPreferredWidth(55);
		tbClientes.getColumnModel().getColumn(util.getFechaNac()).setPreferredWidth(180);
		tbClientes.getColumnModel().getColumn(util.getGenero()).setPreferredWidth(70);
		tbClientes.getColumnModel().getColumn(util.getEstadoCivil()).setPreferredWidth(100);
		tbClientes.getColumnModel().getColumn(util.getCelular()).setPreferredWidth(110);
		tbClientes.getColumnModel().getColumn(util.getCorreo()).setPreferredWidth(250);
		tbClientes.getColumnModel().getColumn(util.getDireccion()).setPreferredWidth(210);
	...
	private void mostrarData(int posFila) {
		...
		for (JComponent j : lista) {
			if (j instanceof JLabel) {
				...
				((JLabel) j).setText(tbClientes.getValueAt(posFila, lista.indexOf(j)) + "");

			} else if (j instanceof JTextField) {

				((JTextField) j).setText(tbClientes.getValueAt(posFila, lista.indexOf(j)) + "");

			} else if (j instanceof JComboBox) {
				String item = tbClientes.getValueAt(posFila, lista.indexOf(j)) + "";
				switch ((j.getName() + "")) {
				...
				default:
					throw new IllegalArgumentException("Unexpected value: " + "ERRORRRRRRRRRRRRRRRRRRRRR");
				}

			} else if (j instanceof JDateChooser) {
				try {
					((JDateChooser) j).setDate(new SimpleDateFormat("yyyy-MM-dd")
							.parse(tbClientes.getValueAt(posFila, lista.indexOf(j)) + ""));
	...
	protected void do_tbClientes_mouseClicked(MouseEvent e) {
		if (DataSelection) {
			mostrarData(tbClientes.getSelectedRow());
		}
	}
	...
	private void validarModificar() {
		if (!txtNombre.getText().matches(Validaciones.NOM_APE)) {
			...
			if (Integer.parseInt(txtEdad.getText()) < 18) {
				JOptionPane.showMessageDialog(txtEdad, "No se aceptan menores de edad");
			}

			if (Integer.parseInt(txtEdad.getText()) > 80) {
				JOptionPane.showMessageDialog(txtEdad, "Nuestras Politicas no permiten mayores de 80 años");
			}
			...
			int result = gCliente.actualizarClientes(temporal);
			if (result != 0) {
	...
	public void validarRegistro() {
		if (!txtNombre.getText().matches(Validaciones.NOM_APE)) {
			...
			Clientes temporal = new Clientes();
			temporal.setCodigo_(9999999);
			temporal.setCodigo(generarCodigoCliente());
			temporal.setNombre(txtNombre.getText());
			temporal.setApellido(txtApellido.getText());
			temporal.setDni(txtDni.getText());
			temporal.setEdad(Integer.parseInt(txtEdad.getText()));
			...
			int result = gCliente.registrarClientes(temporal);
			if (result != 0) {
				registro = true;
				JOptionPane.showMessageDialog(null, "NUEVO CLIENTE REGISTRADO!!", "CLIENTE REGISTRADO EXITOSAMENTE", 1);
			}
		}
	}

	private String generarCodigoCliente() {
		String codCliente = "CL";
		int numCliente = listaPersonas.get(listaPersonas.size() - 1).getCodigo_() + 1;

		if (numCliente < 10) {
			codCliente += "00" + numCliente;

		} else if (numCliente < 100) {
			codCliente += "0" + numCliente;

		} else if (numCliente < 1000) {
			codCliente += numCliente;
		}
		return codCliente;
	}
	...
	protected void do_btnEliminar_actionPerformed(ActionEvent e) {
		...
		if (pos != -1) {
			...
			if (OK == 0) {
				Clientes temporal = new Clientes();
				temporal.setCodigo((String) tbClientes.getValueAt(pos, 0));
				temporal.setNombre((String) tbClientes.getValueAt(pos, 1));
				temporal.setApellido((String) tbClientes.getValueAt(pos, 2));
				gCliente.EliminarClientes(temporal);
				JOptionPane.showMessageDialog(null, "Los datos del Cliente " + temporal.getNombre() + " "
