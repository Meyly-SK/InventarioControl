...
import Interfaces.CamaraWebInterfaceDAO;
import Utils.MySQLConexion8;

public class GestionCamaraWebDAO implements CamaraWebInterfaceDAO{
	...
	public int registrarCamaraWeb(CamaraWeb c) {
		...
		try {
			...
			String sql = "insert tb_camaraweb values (null,?,?,?,?,?,?,?)";
			...
			pstn.setString(4, c.getTipoconex());
			...
			System.out.println("Error en la instruccion SQL REGISTRAR CAMARA WEB " + ex.getMessage());
		} finally {
			try {
				if (con != null)
					con.close();
	...
	public int EliminarCamaraWeb(CamaraWeb c) {
		...
		try {
			...
			// 2. Establecer la instruccion SQL
			String sql = "delete from tb_camaraweb where codigo = ?";
			...
			System.out.println("Error en la instruccion SQL ELIMINAR CAMARA WEB " + ex.getMessage());
		} finally {
			try {
				if (con != null)
					con.close();
	...
	public int actualizarCamaraWeb(CamaraWeb c) {
		...
		try {
			...
			con = MySQLConexion8.getConexion();
			// 2. Establecer la instruccion SQL
			String sql = "{call ActualizarCamaraWeb(?,?,?,?,?,?)};";
			// 3. Preparar la instruccion --> obteniendo comandos SQL
			pstn = con.prepareStatement(sql);
			...
			pstn.setString(3, c.getTipoconex());
			...
			System.out.println("Error en la instruccion SQL ACTUALIZAR CAMARA WEB " + ex.getMessage());
		} finally {
			try {
				if (con != null)
					con.close();
	...
	public ArrayList<Object> leerColumnas() {
		...
		try {
			...
			String sql = "SHOW COLUMNS FROM bd_upmorbsluck.tb_camaraweb;";
			...
			System.out.println("Error >>> En la instruccion SQL LEER COLUMNAS CAMARA WEB " + e.getMessage());
		} finally {
			try {
				if (pstm != null) {
					pstm.close();
	...
	public ArrayList<CamaraWeb> leerCamaraWeb() {
		...
		ArrayList<CamaraWeb> lista = new ArrayList<CamaraWeb>();
		CamaraWeb camWeb;
		...
		try {
			...
			String sql = "{call listarCamaraWeb()};";
			...
			while (res.next()) {
				camWeb = new CamaraWeb();
				camWeb.setCodigo_(res.getInt(1));
				camWeb.setCodigo(res.getString(2));
				camWeb.setMarca(res.getString(3));
				camWeb.setModelo(res.getString(4));
				camWeb.setTipoconex(res.getString(5));
				camWeb.setPrecio(res.getDouble(6));
				camWeb.setStock(res.getInt(7));
				camWeb.setCategoria(res.getString(8));

				lista.add(camWeb);
			}

		} catch (Exception e) {
			System.out.println("Error >>> En la instruccion SQL LEER CAMARA WEB " + e.getMessage());
		} finally {
			try {
				if (pstm != null) {
					pstm.close();
